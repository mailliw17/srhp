<?php
class C_PilihC4 extends CI_Controller{
	...
	function index(){
		...
		$data['tipe'] = 'Ketua Bidang Paguyuban dan Persaudaraan' ;
		...
		$data['cek_hs_dph'] = $data['pemilih']['C4'];

		if($data['cek_hs_dph'] == NULL){
			$hs_dph = 1;
			$data['hs_dph'] = $hs_dph;
			$data['kandidat'] = $this->m_kandidat_dph->get_kandidat4('C4');
		}else{
			$hs_dph = 0;
			$data['hs_dph'] = $hs_dph;
			$data['kandidat'] = NULL;
			$selected_kandidat = $data['pemilih']['C4']; //mengambil data id kandidat ketua wilayah yang dipilih user tsb
			$data['C4'] = $this->m_kandidat_dph->get_kandidat5($selected_kandidat);
		}

		$this->load->view('templates/header_pemilih', $data);
		$this->load->view('pemilih/v_C4', $data);
	...
	public function pilih($id_selected_kandidat){
		...
		$this->m_pemilih->set_C4($nik, $nama, $kode_lingkungan, $kode_wilayah, $id_selected_kandidat);
