...
class C_UsulanKetuaLingkungan extends CI_Controller {
	public function __construct(){
    	...
    	$this->load->model('m_usulan_ketua_lingkungan');
    	$this->load->model('m_pemilih');
   }

	public function index(){

		$data['title'] = 'Usulan Ketua Lingkungan';
		...
		if( $this->form_validation->run() == FALSE){
			...
			$this->load->view('v_usulan_ketua_lingkungan', $data);
			...
			$nama_foto = $lingkungan.'_'.$nama_foto;
        	...
        	if (!empty($_FILES['foto']['name'])) {
	            ...
	            if (!$this->upload->do_upload('foto')) {
	                ...
	                redirect('C_UsulanKetuaLingkungan');
	            } else {
	        ...
	        $id_1 = 'B'.$lingkungan; // setelah ini mencari id terakir/nomer urut paling tinggi dari kandidat dengan id $jabatan+$lingkungan
	        $data['rows'] = $this->m_usulan_ketua_lingkungan->get_row_kandidat($id_1);
	        $total_rows = $data['rows']['jumlah'];
	        if ($total_rows == 0){
	        	$id_final = $id_1.'1';
	        }
	        if($total_rows > 0) {
	        	$data['last_id'] = $this->m_usulan_ketua_lingkungan->get_last_id($id_1);
			...
			$data['nama_sama'] = $this->m_usulan_ketua_lingkungan->cek_nama($nama, $lingkungan, 'B');
			$data['lingkungan'] = $this->m_usulan_ketua_lingkungan->get_lingkungan($lingkungan);
			if ($data['nama_sama']['jumlah'] > 0){
				$this->session->set_flashdata('nama_sama', 'Sudah Ada Sebagai Kandidat Ketua Lingkungan '.$data['lingkungan']['nama_lingkungan']);
	            redirect('C_UsulanKetuaLingkungan');
			}
	        //pengisian data
			$data = array(
        	...
        	$this->m_usulan_ketua_lingkungan->insert($data);
        	$this->session->set_flashdata('kandidat_lingkungan', 'berhasil');
    ...
    public function form_isian2(){
		...
		if( $this->form_validation->run() == FALSE){
			$this->load->view('templates/header', $data);
			$this->load->view('v_usulan_ketua_lingkungan_2', $data);
			$this->load->view('templates/footer');
		} else{//jalankan fungsi
			$nama = $this->input->post('nama', true);
        	...
        	if (!empty($_FILES['foto']['name'])) {
	            ...
	            if (!$this->upload->do_upload('foto')) {
	                $this->session->set_flashdata('gagal_upload_foto', 'Tidak Sesuai Format');
	                redirect('C_UsulanKetuaLingkungan/form_isian2');
	            } else {
	                $foto = $this->upload->data("file_name");
	            }
	        }else{
