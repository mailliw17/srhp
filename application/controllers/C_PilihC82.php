<?php
class C_PilihC82 extends CI_Controller{
	...
	function index(){
		...
		$data['cek_hs_dph'] = $data['pemilih']['C82'];

		if($data['cek_hs_dph'] == NULL){
			$hs_dph = 1;
			$data['hs_dph'] = $hs_dph;
			$data['kandidat'] = $this->m_kandidat_dph->get_kandidat4('C82');
		}else{
			$hs_dph = 0;
			$data['hs_dph'] = $hs_dph;
			$data['kandidat'] = NULL;
			$selected_kandidat = $data['pemilih']['C82']; //mengambil data id kandidat ketua wilayah yang dipilih user tsb
			$data['C8'] = $this->m_kandidat_dph->get_kandidat5($selected_kandidat);
		}

		$this->load->view('templates/header_pemilih', $data);
		$this->load->view('pemilih/v_C82', $data);
	...
	public function pilih($id_selected_kandidat){
		...
		$this->m_pemilih->set_C82($nik, $nama, $kode_lingkungan, $kode_wilayah, $id_selected_kandidat);
